...
if [[ $LAUNCHER == local ]]; then
  ...
  # fast-submit -n 2 -l local --node-driver none ./bin/gam_alloc_test
	...
	fast-submit -n 1 -H hosts -l mpi --node-driver none ./bin/unit_test
	fast-submit -n 2 -H hosts -l mpi --node-driver none ./bin/gam_unit_test
  fast-submit -n 2 -H hosts -l mpi --node-driver none ./bin/gam_alloc_test
	fast-submit -n 5 -H hosts -l mpi --node-driver none --node-driver none ./bin/gff_farm
	fast-submit -n 2 -H hosts -l mpi --node-driver none ./bin/gff_training_mockup
	fast-submit -n 2 -H hosts -l mpi --node-driver none ./bin/gff_training_concurrent
	fast-submit -n 2 -H hosts -l mpi --node-driver none ./bin/gff_training_concurrent_2
	fast-submit -n 9 -H hosts -l mpi --node-driver none ./bin/gff_training_concurrent_grid
	fast-submit -n 1 -H hosts -l mpi --node-driver none ./bin/mxnet_aux_test
	fast-submit -n 2 -H hosts -l mpi --node-driver none ./bin/mxnet_worker_test
fi